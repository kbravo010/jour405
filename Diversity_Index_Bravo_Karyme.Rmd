...
```{r load-data}
md_cities <- read.csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/usat_diversity_index.csv")

# Preview structure
View(md_cities)
```

Each row shows the totals for groups like Black, White, Asian, and Hispanic residents which helps calculate diversity. As these numbers are raw coming from cities like Silver Spring, Hyattsville, and Gaithersburg.
...
City with the highest race probability is likely to have one race that makes up most of its population.
...
Cities with the lowest probabilities scores tend to have a storng mix of Hispanics and non-hispanic residents. This can mean the population is more balanced between the two groups which adds diversity.
...
The least would be the Western side of Maryland where one race makes up majority of the population.
...
Head: Silver Spring Tops Maryland's List of the Most Diverse City

Lede: Data shows that Maryland's clustered near the Washington, D.C.metro area, with Silver Spring leading the state in the most ethnic minorities variety population.

Reporting Plan: I would talk to local residents and the community leaders as well ass city planners. A key question would be how has diversity evolved over the past decade? What challenges comes with such a mix population?
