...
An example I can think of that is not normally distrubted would be hospital visits or incomes. Like for instance, some people incomes are more than others so it is skewed right. Whereas, most people do not normally go to the hospital (mainly because of not having health insurance.) But data like this allows reporters to focus on the median and range instead of the mean since it can become misleading. 
...
Maryland's women's volleyball team had a good performance, while not leading nationally.
...
```{r}
car_thefts |> summarize(mean = mean(count, na.rm = TRUE), sd = sd(count, na.rm = TRUE))
```
...
It is slightly right-skewed with days showing lower theft counts and fewer days with higher counts. The range would be about 68% of the days would fall within the standard deviation.
