...
```{r}
montco_inspections <- read.csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/montco_inspections.csv")
```
...
compliance_stats <- montco_inspections |>
  summarize(
    ...
    sd_compliance_score = sd(compliance_score, na.rm = TRUE),
...
```{r}
ggplot(montco_inspections, aes(x = compliance_score)) +
...
There has been a an increase in compliance scores over the course of time which means inspections of restaurants in MoCo are becoming more better in cleaning and eating area for people. A story here could be "What caused the spike of increase in compliance scores with the number of restaurants in Montgomery County?" Are there new rules and regulations for these restaurants that would prevent them from shutting down?
...
```{r}
md_hs_participation <- read.csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/md_hs_participation.csv")
```
...
```{r}
correlation <- with(md_hs_participation, cor(boys, girls, use = "complete.obs"))
correlation
```
...
```{r}
ggplot(md_hs_participation, aes(x = boys, y = girls)) +
  geom_point(color = "purple", size = 4) +
  labs(
    title = "Relationships Between Boys and Girls Participation in Montgomery County High Schools",
    x = "Number of Boy Participants",
    y = "Number of Girl Participants"
  )
```
...
There is a slight increase with boys and girls participation in High School Sports, however, the data shows a consistentency of an increase. This can be due to the lack of engagement. What can be further incestigates is why is there only one dot at the very top? Does the gap indicate that lack of engagement?
...
```{r}
md_car_thefts |>
  mutate(theft_rate = (X2022 / population) * 10000)
```
...
median_rate <- md_car_thefts |>
  summarize(median_rate = median(theft_rate, na.rm = TRUE)) |>
  pull(median_rate)
```
...
There are a lot of car thefts within Maryland as that has been the most common crime. There has been an increase in the number of car thefts in Maryland, according to the Maryland car theft data.
...
I would use the histogram measure along with measuring the mean, median and sd to varify and calculate this claims. Because we can calculate out of the three which one is used the most.

2. What visualizations would help readers understand the trends? (5 points)
Histogram as it can show the response times of the three categories and which one would have the most call times between the three. Additionally, it seems to have a better visiualization for the readers and a better understanding.
