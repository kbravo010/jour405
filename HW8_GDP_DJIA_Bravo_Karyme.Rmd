...
```{r}
data <- read.csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
correlation <- with(data, cor(DJIA, GDP, use = "complete.obs"))
correlation
...
ggplot(data, aes(x = GDP, y = DJIA)) +
  ...
  labs(
    title = "Relationships of U.S. GDP and DJIA (2013-2014)",
    x = "Gross Products in Trillions (USD)",
...
It shows a strong positive correlation between DJIA and GDP. This can indicate that the economy grows and the stocks rises which are scattered upwards in a sloping pattern meaning that the DJIA have a higher level.
